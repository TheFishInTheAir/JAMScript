...
function esp32Compile(code, cargs) {
    ...
    if('IDF_PATH' in process.env) {
        ...
        try {
            let envPath = JSON.parse(fs.readFileSync(idfEnvPath));
            let installation = Object.keys(envPath.idfInstalled)[0];

            idfPath = envPath.idfInstalled[installation].path;
        } catch (e) {
            printAndExit("Error: couldn't find esp-idf installation. GET KEY"); // TODO: Fill
        }
    }
...
function processArgs() {
    ...
    for (var i = 0; i < args.length; i++) {
        if (args[i].charAt(0) === "-") {
            ...
            } else if (args[i] === "--cside-no-build-cache") {  //TODO: rename
